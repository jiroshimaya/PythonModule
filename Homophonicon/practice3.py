...
class Homophonicon:
  ...
  def execute(cls, text: str, wordlist) -> WordSeqDict:
    ...
    def getWord(word: WordDict) -> ScoreDict:
      ...
      if len(ranks) == 0:
        #return ScoreDict(score=float("inf"), word=WordDict(surface=datetime.datetime.now().strftime("%H%M%S"),token=()), original=word)
    ...
    def dp(l) -> WordSeqDict:
      ...
      for i in range(1,l):
        ...
        if ws1["score"] == float("inf"): continue
        
        original = WordDict(surface=cls.detokenize(tokens[i:l]), token=tokens[i:l])
        w = getWord(original)
        if w["score"] == float("inf"): continue
        #print(i,l,w, w["score"] == float("inf"))
        ws1["score"] += w["score"]
        ws1["words"].append(w)
        results.append(ws1)
  ...
  @classmethod
  def getHomophonicRank(cls, word: WordDict, wordlist: dict[int, list[WordDict]]) -> list[ScoreDict]:
    scores = []
    token1 = word["token"]
